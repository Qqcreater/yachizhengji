...
const parseTeeth = (rawData) => {
  ...
  if (ANATOMY_FILTER_ENABLED.value) {
    parsed = parsed.filter(t => !isAnatomyStructure(t.label))
  }
...
const startAnalysis = async () => {
  ...
  try {
    // 统一通过共享工具发起请求（FormData 传 File 原图，不手动设 Content-Type）
    ...
    // 业务码防御（callOpenApi 已处理 none_img/invalid_img 字符串，这里处理 r 是对象但无牙齿的情况）
    const rObj = responseData.r || responseData.data || responseData.result || {}
    ...
    console.error('=== panoramic 分析异常 ===', e)
...
// ===== 计算属性 =====
const permanentCount = computed(() => teeth.value.filter(t => t.type === 'permanent').length)
const deciduousCount = computed(() => teeth.value.filter(t => t.type === 'deciduous').length)
const unknownCount = computed(() => teeth.value.filter(t => t.type === 'unknown').length)
