 ...
 * - 原图若是透明 PNG 会被 Pillow 判定为 invalid，需重编码为 RGB JPEG
 */
const API_CONFIG = {
  // TODO: 待用户从 openapi-lab 控制台 / 文档确认正确端点
  // 当前已探测：/openapi/v1/{2d,pano,teeth,dental,...}/{p17,p18,...} 均 404
  // 唯一可用：/openapi/v1/x/hneck/p17（头颅侧位片）
...
const startAnalysis = async () => {
  ...
  // ===== 构建请求参数 =====
  // 统一使用 FormData 传 File 原图，浏览器自动生成 multipart boundary
  // 原图若是透明 PNG 会被 Pillow 判定为 invalid，需重编码为 RGB JPEG
  const fileToUpload = await reencodeToJpegIfNeeded(originalFile.value)

  try {
    // 统一通过共享工具发起请求
    // 注意：autocrop 后端期望 img 字段为 base64 dataURL（不是原生 File）
    ...
    if (!result.ok) {
      const msg = formatErrorMessage(result.errClass) || '识别请求失败'
      console.error('=== 识别失败 ===', result.errClass, 'raw:', result.rawText)
      alert(msg)
      return
    }
    ...
    // 后端 r 字段是字符串（业务码 2）
    if (typeof responseData?.r === 'string') {
      const r = responseData.r
      console.error('=== 业务码：r 为字符串 ===', r)
      if (/invalid\s*img|cannot.*identify/i.test(r)) {
        alert('图片文件无法识别\n后端响应：' + r + '\n\n请检查：\n1. 图片文件完整（重新导出一次）\n2. 格式为 JPEG / PNG\n3. 文件大小 < 10MB\n4. 详情请见浏览器控制台')
      } else if (/none\s*img|no\s*image/i.test(r)) {
        alert('未识别到有效口腔影像\n后端响应：' + r + '\n\n请上传标准口腔内照片。')
      } else {
        alert('分析失败：' + r + '\n详情请见浏览器控制台')
      }
    ...
    // 调用 parseResponse 解析；其内部会自行处理业务错误
    try {
      await parseResponse(responseData)
      analysisDone.value = true
      console.log('✓ [AutoCrop] 识别完成')
    } catch (bizErr) {
      const errorMsg = responseData?.msg || responseData?.message || responseData?.r || bizErr.message || '处理失败'
      console.error('❌ [AutoCrop] 业务错误:', errorMsg)
      if (/invalid\s*img|cannot.*identify/i.test(String(errorMsg))) {
        alert('图片文件无法识别：' + errorMsg + '\n\n请重新导出图片后重试。')
      } else {
        alert('分析失败：' + errorMsg + '\n详情请见浏览器控制台')
      }
    }
    ...
    console.log('🏁 [AutoCrop] 请求结束，按钮已恢复')
  }
}

// ===== 工具：把透明 PNG 重编码为 RGB JPEG =====
const reencodeToJpegIfNeeded = async (file) => {
  // 已经是 JPEG 直接返回
