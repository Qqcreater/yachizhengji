...
const startAnalysis = async () => {
  ...
  try {
    // ===== 统一通过共享工具发起请求 =====
    // FormData 传 File 原图；浏览器自动生成 multipart boundary（不手动设 Content-Type）
    // 仅携带 sk 密钥头
    ...
    // ===== 成功处理 =====
    // callOpenApi 已通过 classifyError 统一过滤 none img / invalid img 等业务错误
    // 到这里说明 code === 0 且 r 是有效对象
    const responseData = result.data
    console.log('=== API响应成功 ===')
    console.log('imgsize:', responseData?.imgsize)
    console.log('完整响应:', responseData)

    const rField = responseData?.r || responseData?.data || responseData?.result || responseData?.predictions || {}
    resultImage.value = uploadedImage.value

    console.log('===== API返回数据 =====')
    console.log('rField类型:', typeof rField)
    console.log('rField键值:', rField && typeof rField === 'object' ? Object.keys(rField) : 'N/A')

    landmarks.value = parseLandmarks(rField, responseData?.imgsize)

    if (landmarks.value.length === 0) {
      const allKeys = rField && typeof rField === 'object' ? Object.keys(rField) : []
      if (allKeys.length === 0) {
        alert('识别成功，但模型未检出有效标志点（置信度不足）。\n\n请上传更清晰的 X 光片。')
      } else {
        alert('识别成功，但未返回有效标志点数据。\n\n请检查：\n1. 是否为标准头颅侧位片\n2. 标志点（S/N/A/B/Po）是否清晰可辨\n3. 详情请见浏览器控制台')
      }
    } else {
    ...
    console.error('=== cephalometric 分析异常 ===', e)
    alert('分析过程发生异常：' + (e?.message || String(e)) + '\n详情请见浏览器控制台')
  } finally {
    isAnalyzing.value = false
    console.log('🏁 [Cephalometric] 请求结束，按钮已恢复')
  }
}
